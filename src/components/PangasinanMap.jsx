        ...
        <View style={themeStyle(StyleSheet.absoluteFill)} {...responder.panHandlers}>
          <Svg width="100%" height="100%" viewBox={`${vx} ${vy} ${geometry.width / zoom} ${geometry.height / zoom}`} preserveAspectRatio="xMidYMid meet">
            ...
            {geometry.areas.map((area, i) => (
              <Path key={area.id} testID={`map-area-${area.id}`} d={area.d} fill={selectedIds.includes(area.id) ? colors.sunsetCoral : query && !matches.includes(area) ? mapColors.muted : mapColors.regions[i % 4]}
            ...
            {routePoints.length > 1 && <Polyline points={routePoints.map(s => s.point.join(",")).join(" ")} fill="none" stroke={colors.oceanBlue} strokeWidth={3 / zoom} strokeDasharray="6 4" pointerEvents="none" />}
            {routePoints.map(stop => <G key={`${stop.placeId}-${stop.index}`} pointerEvents="none"><Circle cx={stop.point[0]} cy={stop.point[1]} r={10 / zoom} fill={colors.oceanBlue} /><SvgText x={stop.point[0]} y={stop.point[1] + 4 / zoom} fontSize={11 / zoom} textAnchor="middle" fill={colors.white}>{stop.index + 1}</SvgText></G>)}
